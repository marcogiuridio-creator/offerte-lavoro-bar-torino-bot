...
async def notify_matched_candidates(bot, job_text: str, employer_username: str, group_msg_id: int):
    ...
    for cand in matches[:30]: # Limita ai primi 30 candidati più affini
        ...
        preview_text = job_text[:300] + ("..." if len(job_text) > 300 else "")

        msg = (
            f"🎯 *NUOVA OFFERTA IN TARGET PER TE!* (Affinità: *{score}%*)\n\n"
            f"📝 *Annuncio:* \n_{preview_text}_\n\n"
            f"👤 *Pubblicato da:* {contact_ref}\n"
            f"💡 *Per candidarti:* scrivi direttamente a {contact_ref} o rispondi all'annuncio nel gruppo!"
        )
