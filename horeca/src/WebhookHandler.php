...
final class WebhookHandler
{
    ...
    private function looksLikeJobOffer(string $text): bool
    {
        ...
        $candidateIntent = preg_match('/\\b(cerco|cerca|sto\\s+cercando|sono\\s+in\\s+cerca\\s+di)\\s+(?:un[ao]?\\s+)?(?:lavoro|impiego|occupazione|posizione)|\\bcerco\\s+(?:di\\s+lavorare\\s+)?come\\b|\\bmi\\s+candido|\\bcandidatura\\s+(?:come|per)|\\b(?:sono\\s+)?disponibil\\w*\\s+(?:da|come|per|a\\s+lavorare)|\\b(?:barist\\w*|barman|bartender|camerier\\w*|cuoc\\w*|chef|lavapiatt\\w*|pizzaiol\\w*|banconist\\w*)\\b.{0,40}\\bdisponibil\\w*/u', $normalized) === 1;
        if ($candidateIntent) {
            return false;
        }

        // Le formule esplicite del datore sono gia sufficienti anche quando il
        // ruolo e scritto soltanto come "personale", "staff" o "figure".
        ...
        $role = preg_match('/\\b(?:barist\\w*|barman|barmen|barlady|bartender|mixologist|barback|camerier\\w*|waiter|waitress|operator\\w*\\s+di\\s+sala|runner|commis(?:\\s+di\\s+(?:sala|cucina))?|chef(?:\\s+de\\s+rang|\\s+de\\s+partie)?|demi\\s+chef|capo\\s+partita|cuoc\\w*|aiut\\w*\\s+(?:cuoc\\w*|cucina)|lavapiatt\\w*|plonge(?:ur)?|pizzaiol\\w*|pasticcier\\w*|pastry\\s+chef|sous\\s+chef|sushiman|sushi\\s+chef|grigliator\\w*|gelatier\\w*|panettier\\w*|rosticcier\\w*|banconist\\w*|cassier\\w*|sommelier|hostess|steward|receptionist|portier\\w*\\s+notturn\\w*|camerier\\w*\\s+ai\\s+piani|housekeeping|ma[iî]tre|restaurant\\s+manager|bar\\s+manager|store\\s+manager|direttor\\w*\\s+(?:di\\s+)?(?:sala|ristorante|locale)|responsabil\\w*\\s+(?:di\\s+)?(?:sala|bar|cucina)|supervisor|addett\\w*\\s+(?:di\\s+|al\\s+|alla\\s+|alle\\s+)?(?:sala|banco|bar|cucina|caffetteria|colazioni|accoglienza|pulizie)|personale(?:\\s+(?:di|per))?\\s+(?:sala|bar|cucina|ristorazione)|staff(?:\\s+(?:di|per))?\\s+(?:sala|bar|cucina)|facchin\\w*|tuttofare|extra(?:\\s+sala)?|personale|staff|figur\\w*)\\b/u', $normalized) === 1;
        $offerDetails = preg_match('/\\b(?:inviare|mandare|inoltrare)\\s+(?:il\\s+)?c[vu]|\\bcandidature?\\s+(?:a|al|alla|in|via)|\\bcontratto|\\bretribuzione|\\bstipendio|\\bcompenso|\\bpaga|\\bturn\\w*|\\bfull[ -]?time|\\bpart[ -]?time|\\besperienza\\s+(?:richiesta|gradita)|\\bdisponibilit[aà]\\s+(?:richiesta|immediata)|\\bcontatt(?:are|aci|atemi)|\\bscrivere\\s+(?:in\\s+)?privato|\\bwhatsapp|\\btel(?:efono)?\\b|\\bemail\\b/u', $normalized) === 1;
        $venue = preg_match('/\\b(?:locale|ristorante|bar|pizzeria|pub|caffetteria|trattoria|hotel|albergo|bistrot|gastronomia|gelateria|pasticceria|panetteria|forno|enoteca|cocktail\\s+bar|lounge\\s+bar|mensa|catering|agriturismo|stabilimento|discoteca|club)\\b/u', $normalized) === 1;
        ...
        $location = preg_match('/\\b(?:torino|provincia|centro|san\\s+salvario|san\\s+donato|cit\\s+turin|lingotto|mirafiori|moncalieri|rivoli|collegno|settimo|chieri|via|viale|piazza|corso|zona)\\b/u', $normalized) === 1;
        $score = ($role ? 3 : 0) + ($intent ? 3 : 0) + ($venue ? 1 : 0)
            + ($schedule ? 1 : 0) + ($offerDetails ? 1 : 0) + ($location ? 1 : 0);
        return $score >= 5;
    }
